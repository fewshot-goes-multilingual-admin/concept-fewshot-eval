...
from evaluation.primes_construction import selection_criterion, construct_sample
...
class Evaluator:
    ...
    def collect_predictions(model: PreTrainedModel,
        ...
        cache_inputs_fpath = os.path.join(prediction_cache_dir, "%s-%s-%s-inputs.txt" % identifier)
        cache_expected_fpath = os.path.join(prediction_cache_dir, "%s-%s-%s-expected.txt" % identifier)
        cache_predicted_fpath = os.path.join(prediction_cache_dir, "%s-%s-%s-predicted.txt" % identifier)
        if os.path.exists(cache_expected_fpath) and os.path.exists(cache_predicted_fpath):
            ...
            eval_set_out = []
            num_samples = firstn if firstn is not None else len(eval_set_in)

            skipped = 0
            for batch_offset in tqdm(range(0, num_samples, batch_size), desc="Evaluating %s" % task):
                tuples_batch = eval_set_in[batch_offset: batch_offset + batch_size]
                input_texts = []
                targets = []
